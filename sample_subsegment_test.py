...
class FeatureGenerator(object):
    def __init__(self, dtype, n_jobs=1, chunk_size=None, subchunk_size=None, subchunk_num=None):
        self.chunk_size = chunk_size
        if (subchunk_size==None) or (subchunk_num==None):
            self.subchunk_size = chunk_size
            self.subchunk_num = 1
        else:
            self.subchunk_size = subchunk_size
            self.subchunk_num = subchunk_num
        self.dtype = dtype
        self.filename = None
        self.n_jobs = n_jobs
        self.test_files = []
    ...
    def get_features(self, x_input, y_input, seg_id):
        
        total_dict = dict()
        for i in range(self.subchunk_num):
            x = pd.Series(x_input[i*self.chunk_size/self.subchunk_num, i*self.chunk_size/self.subchunk_num+self.subchunk_size])
            y = y_input[i*self.chunk_size/self.subchunk_num+self.subchunk_size-1]
            ...
            for k, v in main_dict.items():
                if k not in ['target', 'seg_id']:
                    total_dict['sub'+str(i)+'_denoised_'+str(k)] = v

            for k, v in r_dict.items():
                if k not in ['target', 'seg_id']:
                    total_dict['sub'+str(i)+'_fftr_'+str(k)] = v
                
            for k, v in i_dict.items():
                if k not in ['target', 'seg_id']:
                    total_dict['sub'+str(i)+'_ffti_'+str(k)] = v
            ...
            for k, v in r_dict_denoised.items():
                if k not in ['target', 'seg_id']:
                    total_dict['sub'+str(i)+'_denoised_fftr_'+str(k)] = v
                
            for k, v in i_dict_denoised.items():
                if k not in ['target', 'seg_id']:
                    total_dict['sub'+str(i)+'_denoised_ffti_'+str(k)] = v
...
def train_subsegment_model(X, X_test, y, n_fold, params=None, model_type='lgb', name='lgb', 
                           plot_feature_importance=False, model=None, subsegment_num=1):
...
def master_test(preparation_threads):
    ...
    try:
        ...
        training_fg = FeatureGenerator(dtype='train', n_jobs=10, chunk_size=150000, subchunk_size=)
